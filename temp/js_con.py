...
class JoystickController:
    def __init__(self):
        ...
        self.disable_servo_control = rospy.get_param(
            '~disable_servo_control', 'true')
        cmd_vel = rospy.get_param('~cmd_vel', 'jetauto_controller/cmd_vel')

        self.joy_sub = rospy.Subscriber(
            'joy', sensor_msg.Joy, self.joy_callback)
        self.mecanum_pub = rospy.Publisher(
            cmd_vel, geo_msg.Twist, queue_size=1)

        self.last_axes = dict(zip(AXES_MAP, [0.0,] * len(AXES_MAP)))
        self.last_buttons = dict(zip(BUTTON_MAP, [0.0,] * len(BUTTON_MAP)))
        self.mode = 0

        self.joint1 = rospy.Publisher(
            'joint1_controller/command_duration', CommandDuration, queue_size=1)
        self.joint2 = rospy.Publisher(
            'joint2_controller/command_duration', CommandDuration, queue_size=1)
        self.joint3 = rospy.Publisher(
            'joint3_controller/command_duration', CommandDuration, queue_size=1)
        self.joint4 = rospy.Publisher(
            'joint4_controller/command_duration', CommandDuration, queue_size=1)
        self.jointr = rospy.Publisher(
            'r_joint_controller/command_duration', CommandDuration, queue_size=1)
        ...
        self.joint5_state = 0
        self.jointr_state = 0
        rospy.Subscriber('joint1_controller/state', JointState,
                         lambda msg: setattr(self, 'joint1_state', msg.goal_pos))
        rospy.Subscriber('joint2_controller/state', JointState,
                         lambda msg: setattr(self, 'joint2_state', msg.goal_pos))
        rospy.Subscriber('joint3_controller/state', JointState,
                         lambda msg: setattr(self, 'joint3_state', msg.goal_pos))
        rospy.Subscriber('joint4_controller/state', JointState,
                         lambda msg: setattr(self, 'joint4_state', msg.goal_pos))
        rospy.Subscriber('jointr_controller/state', JointState,
                         lambda msg: setattr(self, 'jointr_state', msg.goal_pos))

    def axes_callback(self, axes):
        twist = geo_msg.Twist()
        twist.linear.y = misc.val_map(
            axes['lx'], -1, 1, -self.max_linear, self.max_linear)
        twist.linear.x = misc.val_map(
            axes['ly'], -1, 1, -self.max_linear, self.max_linear)
        twist.angular.z = misc.val_map(
    ...
    def start_callback(self, new_state):
        if new_state == ButtonState.Pressed:
            if self.mode == 0:
                ...
                self.joint2.publish(CommandDuration(
                    data=math.radians(-40), duration=2000))
                self.joint3.publish(CommandDuration(
                    data=math.radians(110), duration=2000))
                self.joint4.publish(CommandDuration(
                    data=math.radians(70), duration=2000))
